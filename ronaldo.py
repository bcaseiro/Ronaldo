...
#           argv0    argv1   argv2  
# python ronaldo.py system /tmp/ips.txt
#Please enter the password for the user <user>:
	...
	for line in lines:
		try:
			...
			for privuser in cursor2:
				...
				trata1 = line.split('\n')
				trata3 = str(trata1[0])
				#print trata_result2, trata3, oracle_version
			...
			#for abc in lines:
				...
				#cursor.execute("SELECT username FROM v$pwfile_users")	

				#All privileged users are here
	    	
	    			#privusers = cursor
	   			
	    		
				##for puser in privusers:
				#	trata_puser = ''.join(puser)				
				#	print (trata_puser)
			...
			print ('')
			#print ('Connection error with the server: ', line, err)
...
#cursor = con.cursor()
#select username from dba_users where account_status like 'OPEN';""")
